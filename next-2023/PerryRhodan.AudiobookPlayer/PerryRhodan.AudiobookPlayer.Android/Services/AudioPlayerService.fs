...
module rec AudioPlayerServiceImplementation =
    ...
    module Notification =
        ...
        let private buildIntentToShowMainActivity () =
            let notificationIntent = new Intent(context, typeof<AppCompatActivity>)
        ...
        let buildNotification (mediaSession:MediaSession) info =            
            ...
            // build proper media controls for android
            ...
            let currenInfoString = 
                if Build.VERSION.SdkInt < BuildVersionCodes.Q then
                    $"""{info.CurrentTrackNumber} - {(info.Position |> Common.TimeSpanHelpers.toTimeSpan).ToString("mm\:ss")} / {(info.Duration |> Common.TimeSpanHelpers.toTimeSpan).ToString("mm\:ss")}"""
    ...
    module Services =
        ...
        [<Service(Exported=true,Name="perry.rhodan.audioservice",ForegroundServiceType=PM.ForegroundService.TypeMediaPlayback)>]
        type AudioPlayerService() as self =
            ...
            override this.OnStartCommand(intent,_,_) =
                ...
                if intent = null || intent.Action = null || intent.Action = "" then
                    ...
                    | _ ->
                        Crashes.TrackError(exn $"AudioPlayerService: unknown intent action. '%s{intent.Action}'", Map.empty)
